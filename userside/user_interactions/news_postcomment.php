    ...
    if(!isset($_SESSION["username"])){
        ...
        if($fileNames){
            foreach($_FILES["imgs_comment"]["name"] as $key=>$val){
                ...
                if(in_array($fileType, $allowTypes)){
                    ...
                    if(move_uploaded_file($_FILES["imgs_comment"]["tmp_name"][$key], $targetFilePath)){
                        ...
                        $insertValuesSQL = "('$user_id','$_POST[txt_news_id]','".mysqli_real_escape_string($conn, $_POST["txt_content"])."', '".$fileIMGs."', '$dateNow')";
                    }else{
            ...
            if(!empty($insertValuesSQL)){
                ...
                $insert = $conn->query("insert into tbl_newscomments (user_id, news_id, fld_content, fld_commentimages, fld_datetime) values $insertValuesSQL");
                if($insert){
