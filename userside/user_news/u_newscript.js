...
$(function(){
    ...
    $(document.body).on('click', '.news_container', function(){
        window.location.href = "u_news.php?news_category="+$(this).attr("news_category")+"&news_id="+$(this).attr("news_id");
    });

    $("#btn_back").click(function(){
        window.location.href = "u_news.php?news_category="+$(this).attr("news_category");
        
    });

    $(document.body).on("click",".btn_bookmark",function() {
       
		Bookmark(parseInt($(this).attr("news_id")));

	});

    $(document).on('click', '.admin_result', function(){
        var pattern = /news_category/;
    ...
    $(document.body).on("click","#btn_savereply",function() {
        AddReply(parseInt($(this).attr("comment_id")), news_id);
    });
...
function Bookmark(news_id){
    var c_id = "";

    c_id = "news_id=" + news_id;
    ...
    $.ajax({
        ...
        "success": function(text){
            if(text == "not login"){
                ...
                $("[news_id="+news_id+"]").prop("src", bookmark.url);
            }
...
function AddComment(formData, news_id){
    console.log("ok");
    $.ajax({
        type:'POST',
        url:'../user_interactions/news_postcomment.php',
        data:formData,
        processData: false,
        contentType: false,
        dataType: 'json',
...
function ViewComments(news_id){
    
    var cNews_id = "";

    cNews_id = "news_id=" + news_id;

    $.ajax({
        "type": "POST",
        "url": "../user_interactions/loadnewscomments.php",
        "data": cNews_id,
        "success":function(text){
...
function RemoveComment(comment_id){
	...
	$.ajax({
		"type":"POST",
		"url": "../user_interactions/deletenewscomment.php",
		"data": cParam,
		"success":function(text) {
...
function ViewReplies(comment_id){
    ...
    cComment_id += "&news_id=" + news_id;

    $.ajax({
        "type": "POST",
        "url": "../user_interactions/loadnewsreplies.php",
        "data": cComment_id,
        "success":function(text){
...
function AddReply(comment_id, news_id){
    cReply = "";

    cReply = "txt_reply="+$("#txt_reply"+comment_id).val();
    cReply += "&comment_id="+comment_id;
    cReply += "&news_id="+news_id;

    console.log(news_id);

    $.ajax({
        "type":"POST",
        "url": "../user_interactions/savenewsreply.php",
        "data": cReply,
        "success": function(text){
            if(text !== ""){
                if(text == "not login"){
...
function RemoveReply(reply_id, comment_id){
	...
	$.ajax({
		"type":"POST",
		"url": "../user_interactions/deletenewsreply.php",
		"data": cParam,
		"success":function(text) {
